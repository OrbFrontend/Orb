...
from backend.inference.prose_rewriter import catalog, profile
...
async def test_a_variant_outside_the_registry_is_refused(downloaded):
    ...
    with pytest.raises(ValueError, match="Unregistered prose-rewriter variant"):
        profile.launch_profile_for(forged, True, 2)
...
async def test_a_batch_size_outside_the_supported_range_is_refused(downloaded, batch_size):
    with pytest.raises(ValueError, match="slots must be between 1 and 4"):
        profile.launch_profile_for(catalog.variants()[0], True, batch_size)
...
async def test_a_variant_with_no_file_behind_it_says_so(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="is not downloaded"):
        profile.launch_profile_for(variant, True, 4)
...
async def test_launch_profile_is_a_pure_function_of_the_selection(downloaded):
    ...
    built = profile.launch_profile_for(variant, True, 2)

    assert built == profile.launch_profile_for(variant, True, 2)
    assert built != profile.launch_profile_for(other, True, 2)
    assert built != profile.launch_profile_for(variant, False, 2)
    assert built != profile.launch_profile_for(variant, True, 3)
...
async def test_parallel_slots_select_only_fixed_command_arguments(downloaded, monkeypatch, batch_size):
    ...
    built = profile.launch_profile_for(catalog.variants()[0], True, batch_size)
    ...
    assert argv[argv.index("--ctx-size") + 1] == str(batch_size * profile.CTX_PER_SLOT)
...
async def test_gpu_placement_is_one_number_on_the_command_line(downloaded):
    variant = catalog.variants()[0]
    assert profile.launch_profile_for(variant, True, 1).gpu_layers == 999
    assert profile.launch_profile_for(variant, False, 1).gpu_layers == 0
...
async def test_a_selection_with_nothing_behind_it_is_a_profile_of_none(tmp_path, monkeypatch):
    ...
    assert profile.profile_for_selection(None, True, 4) is None
    assert profile.profile_for_selection(catalog.variants()[0], True, 4) is None


@pytest.mark.parametrize("raw", [1, 2, 3, 4])
async def test_batch_size_selector_maps_supported_input_to_the_closed_allowlist(raw):
    assert profile.select_batch_size(raw) == raw
...
async def test_batch_size_selector_rejects_everything_outside_the_closed_allowlist(raw):
    assert profile.select_batch_size(raw) is None


# ── the persisted blob → the turn's config ───────────────────────────────────
# Still resolved by the pipeline adapter for one more phase; Phase 3 of the
# local-model split moves `resolve_prose_rewrite` into this feature.
