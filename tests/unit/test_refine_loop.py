...
from backend.passes.refine.slop_detector import DetectionResult, FlaggedSentence, ClicheHit
...
def _dirty_report(n_issues: int, label: str = "") -> AuditReport:
    ...
    flagged = [
        FlaggedSentence(
            ...
            cliches=[ClicheHit(canonical="test_phrase", variant="test_phrase", score=1.0)],
        )
    ...
    return AuditReport(
        ...
        monotony_result=MonotonyResult(
            flagged_openers=[], all_openers={}, total_sentences=n_issues, monotony_score=0.0
        ),
        template_result=TemplateResult(
            flagged_templates=[], all_templates={}, total_sentences=n_issues,
            unique_templates=0, repetition_score=0.0,
        ),
        not_but_result=[],
    )
...
def _make_client(messages_per_call: list[dict]) -> MagicMock:
    ...
    async def _complete(*args, **kwargs):
        idx = call_idx[0]
        call_idx[0] += 1
        msg = messages_per_call[idx] if idx < len(messages_per_call) else messages_per_call[-1]
...
async def _run(client, audit_side_effects):
    ...
    with patch(
        ...
        return [
            event async for event in refine_pass(
                client=client,
                prefix=[],
...
class TestRefineLoopTermination:
    ...
    async def test_updated_audit_report_sent_each_iteration(self):
        ...
        audit_side_effects = [
            ...
            (_dirty_report(2), "REPORT_LABEL_2"),   # post-iter-0 → becomes tool-result for iter-1
            (_dirty_report(1), "REPORT_LABEL_1"),   # post-iter-1 → becomes tool-result for iter-2
            (AuditReport.clean(), "clean"),          # post-iter-2 → loop breaks
        ]

        captured_msgs: list[list[dict]] = []
        call_idx = [0]
        patches_by_iter = [("alpha", "ALPHA"), ("bravo", "BRAVO"), ("charlie", "CHARLIE")]
        ...
        assert client.complete.call_count == 3, "Expected 3 iterations before audit cleared"
    ...
    async def test_hits_max_iterations_when_issues_persist(self):
        ...
        assert client.complete.call_count == MAX_REFINE_ITERATIONS, (
            f"Expected exactly {MAX_REFINE_ITERATIONS} LLM calls when the turn limit is reached"
        )
        assert done["draft"] is not None, "Draft should be changed after patches were applied"
