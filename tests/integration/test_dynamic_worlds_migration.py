"""Upgrade path for Dynamic Worlds (migration 0053).
...
def test_worlds_gain_their_opt_in_and_revision_stamp(tmp_path):
    ...
    try:
        assert conn.execute(
            "SELECT dynamic_enabled, content_revision FROM worlds"
        ).fetchone() == (0, 0)
    finally:
        conn.close()
...
def test_the_migration_is_idempotent(tmp_path):
    ...
    try:
        ...
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM lorebook_entries WHERE entry_layer = 'authored'"
            ).fetchone()[0]
            == 2
        )
    finally:
...
async def test_existing_database_migrates_before_latest_schema_indexes_run(
    ...
    try:
        columns = {
            row[1] for row in conn.execute("PRAGMA table_info(lorebook_entries)")
        }
        indexes = {
            row[1] for row in conn.execute("PRAGMA index_list(lorebook_entries)")
        }
        assert "entry_layer" in columns
        assert "idx_lorebook_overlay" in indexes
        assert conn.execute(
            "SELECT COUNT(*) FROM schema_migrations WHERE id = '0053_dynamic_worlds'"
        ).fetchone() == (1,)
    finally:
        conn.close()
...
def test_upgraded_fk_shape_matches_a_fresh_install(tmp_path):
    ...
    try:
        ...
        for table in ("worlds", "lorebook_entries", "world_changesets"):
            ...
            assert presets._edge_set(live.tables[table]) == presets._edge_set(
                canon.tables[table]
            ), table
