...
- **Editor patching:** `editor_apply_patch` anchors on a numbered finding id, not a `search` string — `analysis/targets.py` resolves the audit into addressable offsets. Every replacement is healed before it lands (`analysis/healing.py`): any run of words the model copied from the draft *outside* its target span is trimmed off either end, so a mis-aimed patch can't print the same text twice. What healing leaves is then guarded (`analysis/guarding.py`): the text between and around the targets is protected, and a replacement that clones a significant lexical run out of it — the same copy, sitting where no trim could safely remove it — is rejected per-patch, keeping the writer's original text for that target. The guard is format-agnostic (quotes, smart quotes, asterisks and bare lines all fold to the same tokens) and classifies nothing; paraphrase and prose quality are not its job.
...
| `analysis/` | Pure prose-quality detection: `audit.py` + detectors, `targets.py` (findings → id-addressable draft offsets), `patching.py`, `healing.py` (trims patch text that restates the draft around the span), `guarding.py` (rejects a patch that clones protected text between the targets); shared by editor + workflows |
...
| `messages` | Message tree (`parent_id`); group replies carry `speaker_member_id` and request-scoped `exchange_id`; Writer-produced replies retain inline-macro-frozen pre-rewriter/editor `writer_draft` for on-demand local rewriting — immutable except that a hand edit clears it, since it then describes text that no longer exists; a row without one rewrites its own content |
