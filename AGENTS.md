...
| `member_sheet_proposals` | Staged rewrites of `group_members.card_sheet_override`, **at most one `pending` per member**; `pending`\|`applied`\|`rejected`\|`stale`. Written by the post-exchange pass, decided by the user in Manage cast; retired with the member when the roster drops it |
