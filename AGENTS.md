...
- **Community extensions:** Untrusted declarative packages, a separate trust tier from built-in workflows. `.orbext` archives and HTTPS Git URLs are compiled to immutable records, stored content-addressed under `data/extensions/objects/<digest>/`, and published as the registry's community overlay. Compiled flows run through a bounded interpreter with a staged effect transaction; compiled views render through a host-owned component renderer that only ever writes `textContent`. **The unit of permission is a `(capability, parameter)` grant, and every fact about one — consent copy, data class, emphasis, admissible parameter values, gated resource, prerequisites — lives in `CAPABILITY_SPECS` (`features/extensions/contracts/capabilities.py`); the consent table, loud/data-reading sets, resource map, prerequisite map, `UI_SLOTS`, and the `Permission` model's parameters are all derived from it. Add a grant there, not in six places.** `extension_api` is versioned by a table, not a literal: `SUPPORTED_EXTENSION_APIS` says which versions this build implements and `CONTRIBUTION_MIN_API` says which version each `contributions` slot was introduced in, so a v1 manifest carrying a v2 slot is rejected rather than silently upgraded. One doc covers the whole contract, for authors and for growing the ABI: [docs/architecture/community-extensions.md](docs/architecture/community-extensions.md).
- **Community Writer tools (API 2):** The one exception to "community packages never touch the main tool blob". A v2 package may contribute at most one Writer tool behind `writer.tool.contribute`; the user selects at most one *active resolver* across the whole install. Ownership is split three ways and stays split: `core/writer_tools.py` owns the ABI values, call ceiling, host invocation ordinal, and derived provider-facing name (a package never declares one), `workflows/` carries the `WriterToolBinding` and turn-budget ledger, `features/extensions/` compiles the executor from a package flow, and `pipeline/` sends the schema and invokes the binding it captured — so `pipeline/` still never imports the extension feature. `_PipelineConfig` keeps `agent_tool_schemas` and `writer_tool_schemas` apart: single-model builds one deterministic union (agent order preserved, Writer schema appended), dual-model gives each lane only its own. The Writer runs a bounded interleaved ReAct loop — up to `MAX_WRITER_TOOL_CALLS_PER_TURN` tool-bearing completions plus one final reaction, with the configured Writer output tokens and extension operation quotas shared across the whole turn. Only *standard structured* `tool_calls` count; malformed nested calls recover cleanly, content-encoded fallbacks are unsafe once prose has streamed, and non-retryable failures force the next completion to `tool_choice="none"`. The tail OOC block is fixed host text with two bounded package-influenced holes, and it is the semantic tail even with attachments. Full contract: [docs/architecture/community-extensions.md](docs/architecture/community-extensions.md#writer-tools-extension_api-2).
- **Community audit detectors (API 3):** The Editor's mirror of the Writer tool, with the same three-way ownership split and the same trust profile. A v3 package may contribute checks to the Output Auditor behind `audit.detector.contribute`; their findings merge into the same `AuditReport` the built-in scanners produce, so they render in the Editor's report *and* feed the prefilled `editor_apply_patch` path. `ContributedFinding` lives in `analysis/audit.py` — no new `core/` module, because `analysis/` already sits below every owner that must agree on it. The finding shape is host-fixed (`{snippet, note}`; the package declares no output schema) and `label` is stamped by the host from the binding. **Default off**, unlike `analysis.audit._on`: installing a package must not silently add a per-turn model call and a draft-shaped egress to every reply — one extra click, keyed into the existing `settings.editor_audit_toggles` JSON under `<ext>:<local>`. Detectors run **once per turn** under one `asyncio.wait_for`; `filter_audit_report_to_text` ages findings out across the rewrite loop, which is what makes once-per-turn correct. Any failure yields zero findings and never fails the turn. `OpContext.DETECTOR` joins `EXTERNAL_CONTEXTS` — model calls, state, HTTP, but never `ui.*`, `draft.replace`, artifacts, or message-scoped state (no assistant row exists yet). Document mode is deliberately out of scope. Full contract: [docs/architecture/community-extensions.md](docs/architecture/community-extensions.md#audit-detectors-extension_api-3).
...
- `writer_tools.py` owns only the Writer-tool ABI values and the pure
  invariants over them: the call ceiling, host invocation ordinal, derived
  provider-facing name, call-id validity, and fixed result/error encoding.
  Admitted because `workflows/`,
...
| `backend/pipeline/passes/writer.py` | Writer content assembly, the tail OOC policy, and the bounded interleaved Writer ReAct loop |
...
| `backend/core/writer_tools.py` | Writer-tool ABI: call ceiling, `WriterToolSpec`/`Invocation`/`Result`, derived wire names, fixed result codes |
...
- **Storage:** `GET /api/storage?days=N` (what a cleanup would reclaim, plus the extension content store's revision count and bytes — reported only; that content is freed by uninstall, not by age), `POST /api/storage/cleanup` (age-based artifact eviction + Director-log wipe — payload columns blanked in place, `LOG_KEEP_COLUMNS` whitelist survives — then VACUUM)
...
### Contribute an audit detector (extension API 3)
Set `extension_api: 3`, request `audit.detector.contribute`, and declare one or more `contributions.audit_detectors` entries with an `id`, `label`, optional `description`, and `flow`. The flow compiles under `OpContext.DETECTOR` — the same allowlist as a Writer tool (pure ops, namespaced `state.*`, `model.*`, `http.request`, `return`; no `ui.*`, no `draft.replace`, no `artifact.emit`, no message-scoped state) — and must return the host-fixed `[{snippet, note}]` shape; there is no package-declared output schema and no `config_schema` (use namespaced `state.*`). `ctx` may carry `draft` and `direction` behind their `context.read` grants. Installing publishes the binding; the user still has to tick it in the Output Auditor panel, which writes `settings.editor_audit_toggles["<ext>:<local>"]`. Test fixture: `audit_detector_package()` in `tests/extension_packages.py`.
