...
// see the `gaps` field on ProviderPreset for why they are not per-render notes.
...
// Both halves of the form, together. Every caller that re-renders needs both — a
// style row and a connection row can each hold an uncommitted edit — so pairing
// them here is what stops a new call site remembering only one and dropping the
// other's live values on the next innerHTML swap.
