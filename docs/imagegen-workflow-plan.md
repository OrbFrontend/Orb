...
The slot map comes from a picker, never inference: Orb lists candidate nodes — `STRING` inputs for positive/negative, seed-named `INT` inputs for the seed slot, `output_node` classes for the image to fetch — typed from the server's `/object_info`, and the user assigns each role. *(Implementation note 2026-07-21: `/object_info` is tens of megabytes on a real install, so it is never handed to the browser. `POST /api/workflows/image_gen/external/node-types` takes the graph's class-type names and returns only the role verdict per class; the typing rule lives beside the validation that reads the same catalogue. A class the server does not know is simply absent, and the picker degrades to conventional input names for it. The `negative` role is **optional** — a prose-trained graph has one encoder and no negative conditioning, and mapping the role anyway would patch text into an unrelated input.)* The picker steers toward a `SaveImage`-class output; `PreviewImage` outputs are `temp`-type and ephemeral on the server, which still fetches correctly (Orb persists the bytes) but is the fragile choice. Candidates are labelled by the graph's own `_meta.title`, falling back to class type and node id — "Positive Prompt (CLIPTextEncode #6)", never a bare "6". When a role has exactly one plausible candidate the picker preselects it, and every role still requires explicit confirmation before save: preselection is a default inside the picker, so the invariant holds — the map is user-confirmed, never silently inferred.
...
Every hook calls `normalize_config` because workflow `config_schema` is UI metadata, not enforcement. *(Correction 2026-07-21: read-side normalization alone is not enough. The generic config routes stored and echoed the raw body, so a graph the normalizer later dropped stayed visible in the settings panel as a workflow the render path silently ignored. The `Workflow` record now carries an optional `config_normalizer` that both config routes apply — the write path persists the canonical shape and the response is what the hooks will actually read.)* Validation includes the source/style enums, bounded strings, HTTP(S) URLs, timeout range, and source-specific required fields. The normalizer drops unknown keys, coerces numerics that round-tripped through JSON as strings, and returns a new canonical dict merged over the defaults, so a partial or empty persisted slot still resolves every key and downstream code reads typed values without rechecking.
...
**Off-turn paths need the full prefix, not a bare transcript.** On-demand and regenerate run with no turn in flight, so they have no pipeline prefix to reuse. Since generation is on-demand only, *every* composition path is off-turn — there is no in-turn variant to fall back on. A plain role/content list of history omits the system prompt and character framing, and a composer that cannot see who the character is describes a generic person. These paths must rebuild the prefix a pipeline pass would receive: effective system prompt, character persona and scenario, example messages, macros, resolved persona description, constant lorebook block, and history up to the anchor message only. *(Correction 2026-07-22: this paragraph originally accepted forgoing KV reuse and paying the cold prompt. That waste is not accepted — the off-turn prefix is rebuilt **byte-identical** to the pipeline's, including the constant lorebook block, so the server's cached conversation KV survives analyze → compose → the next chat turn. The parity is pinned by `tests/integration/workflows/test_offturn_prefix_parity.py`; per-call instructions ride only the tail messages after the shared prefix, and the forced calls pass `tools_in_prompt=False` so no per-call tool schema can perturb the server-rendered prompt in chat mode either. The one accepted gap remains pre-pipeline workflow system blocks, which no hook currently emits.)*
...
Declare `image_gen_workflow` with `produces_artifacts=True` and bind ON_DEMAND, REGENERATE, and REROLL_GEN in `backend/workflows/__init__.py` before `finalize_registry()`. **No POST_PIPELINE binding**: nothing in this workflow runs inside a turn, so it cannot delay assistant persistence, cannot hold pipeline locks, and cannot fail a turn. This is the largest single simplification in the design, and several consequences follow from it — every composition path is off-turn, so there is exactly one prefix-construction path rather than an in-turn one and an off-turn one that must produce comparable results; and there is no turn-cancellation propagation to handle. *(Correction 2026-07-22: the original conclusion "no KV-cache question arises" was wrong — the off-turn calls share the llama.cpp server's per-conversation KV with the chat turns, so the off-turn prefix must be byte-identical to the pipeline's; see the correction under "Prompt composition".)*

### On demand

`POST .../workflows/image_gen/trigger` action `generate` accepts `{message_id, style_id}`. Validate that `message_id` is an integer but not a boolean, that the target message exists in this conversation and is an assistant message, that it sits on the conversation's **active branch** (conversation membership is not branch membership: an off-path anchor would otherwise compose the image from a history slice that never reaches it, describing replies that came *after* the one being visualized), and that `style_id` resolves for the active source — a live catalog id in managed mode, an entry in the user's external style list in external mode. Compose from the message, resolve the selected style/source, generate, and insert one workflow attachment.
...
- The body is guarded in full. An uncaught exception inside a streaming response aborts the chunked transfer without its terminating chunk, leaving the client's reader waiting on a stream that never closes; degrading to a null result keeps the UI unblocked. *(Correction 2026-07-21: the terminal frames must be emitted after the `try`/`except`, never from a `finally`. A client that disconnects mid-render closes the generator, which throws `GeneratorExit` at the suspended yield, and yielding from `finally` under that raises `RuntimeError: async generator ignored GeneratorExit`. `finally` is for cancelling the render task and nothing else. `CancelledError` is a `BaseException` and is deliberately not caught.)*
- The stream ends on an explicit terminal event carrying the new attachment id, or null when generation produced nothing. Clients finish on that event rather than on stream close, which can stall. **Every rejection uses this wire, including the pre-render guards** — a guard that answers with a plain JSON body leaves the client parsing it as SSE, finding no frames, seeing no terminal event, and silently re-enabling its button with nothing shown to the user.
...
- **Both resolve their render target from the stored row, not from the style.** `resolve_render_target(config, style_id, replay)` prefers the recorded `workflow_id`/`backend_model`; only a fresh generate follows the style's current pins. Resolving replay through the style re-renders an old attachment on whatever checkpoint that style points at today — and rehydrate, which promises to restore the row's own bytes, would overwrite it with a different image and report success. A recorded graph that no longer exists degrades to the style's workflow *with a note on the attachment*, never silently.
...
- an unreachable server replaces Generate with `Can't reach ComfyUI at <host:port>` plus Open settings and Retry, decided by a briefly cached readiness probe when the modal opens (a reuse of `POST /connections/test`, not a new route). *(Implementation note 2026-07-21: what is cached is `/object_info` — the one enormous response in the contract, tens of megabytes on the probe's 1953-node install — behind a 60-second TTL keyed by server URL. The probe sends no config and may answer from it; an explicit Test connection sends the unsaved form config and always refetches, because pressing Test means "look again". Everything else in the probe is small enough to re-run.)*
