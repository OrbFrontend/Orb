...
| 11 | `world_change_proposed` | BE→FE | `{ "message_id": 412, "changeset": {...} }` | *Optional.* The Agent staged a **pending** Dynamic Worlds proposal against this reply. Nothing is applied and nothing is lore yet; FE flags a full repaint so the review card paints under the finished bubble. Emitted after the assistant row is persisted and before `done`. |
