...
- **Dynamic schemas are built once per turn, not once per pass.** `direct_scene`, `give_feedback`, and `record_direction_note` are all assembled at runtime from the user's enabled interactive fragments. Core and extension-defined fragment types resolve their property schemas and instance configuration against the turn's captured registry snapshot before this assembly. Each schema is built one time per turn from that immutable fragment set and then threaded through every pass. Prior/reduced values stay in trailing messages, never in the schema, so the same fragment configuration produces the same schema bytes turn after turn.
...
- **A selected community Writer tool joins the blob as one appended entry.** This is the single exception to "community packages never touch the main tool blob" (see [Community Extensions](community-extensions.md#writer-tools-extension_api-2)), and it is bounded on both sides. Only *one* contribution is ever sent — the user's selected resolver, resolved from the turn's captured registry snapshot — and the union is assembled in `_union_tool_schemas` as `agent schemas, then the Writer schema`. Agent order is preserved exactly, so an install with no selected resolver produces byte-identical blobs to a build without the feature; selecting one perturbs the tail rather than reshuffling the prefix. The schema is compiled at install time and pinned to a content digest, so it is byte-stable within a captured generation: a turn cannot send one revision's schema and invoke another's. Selecting, deselecting, or updating a resolver *intentionally* changes the blob and forks the cache — that is the user changing what the model can do, and the fork is the honest cost of it.
...
### Invariant 5 — When the agent uses a separate model, each lane carries only its own tools

If the director and editor are configured to run on a different model than the writer, the writer's KV cache lives on a different inference server and can't be shared with the agent passes. In that case the writer sends no *agent* tools — including them would just waste tokens with no caching benefit. The agent passes still share a cache with each other on their own server.

The rule is per lane, not "the writer sends nothing". A dual-model writer sends exactly the schemas it may actually invoke, which today means the one selected community Writer tool and nothing else; the agent lane sends the agent tools and never the Writer's. That distinction was invisible until Writer tools existed — every tool in the registry was an agent tool, so "the writer's own set" and "empty" were the same list — and `_PipelineConfig` now keeps `agent_tool_schemas` and `writer_tool_schemas` apart so the coincidence cannot be mistaken for the invariant.
...
### When the writer used a tool: what "the writer exchange" means

A turn in which the Writer called its one selected community tool does not end with a single assistant message. The Writer's own transcript is:

```
user(writer request) → assistant(prose so far + tool_call) → tool(result) → assistant(continuation)
```

Every downstream agent-lane call — editor, feedback, post-turn direction notes — asks `TurnState.writer_replay()` which of two shapes to send, and the answer is decided by topology rather than by convenience:

- **Single-model:** replay the sanitized trace. The shared base already declares that tool, so those four messages are exactly the bytes the Writer just warmed, and continuing them is the same extend-don't-fork rule the feedback step follows. The request carries a host-authored canonical-draft block, because the assistant message immediately above may hold only the post-tool continuation and the editor matches patches against the *whole* draft.
- **Dual-model:** replay the normalized `user(request) + assistant(final draft)` pair, exactly as before Writer tools existed. The agent base deliberately does not declare the Writer's tool, so a historical call to it would be a message that model was never told about — and there is no Writer-lane cache on that server to extend by replaying it.

A turn with **no** tool call takes the normalized path in both modes, so the shapes above are additions to the matrix, not a rewrite of it.

The trace is ephemeral. Only the concatenated final prose is persisted, so the *next* turn's prefix is the ordinary `[system, m1, …, user_N, asst_N]` — it cannot byte-extend the complete interactive transcript from the prior request. That cross-turn fork after the shared prefix is accepted: persisting hidden tool messages to chase a cache hit would put protocol scaffolding into the conversation the user reads, branches from, and exports.
